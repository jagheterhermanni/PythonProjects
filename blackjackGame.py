def play_round():
    ...
    while len(d_cards) != 2:
        ...
        if len(d_cards) == 2:
            print("The cards dealer has are", d_cards[1])  # shows 1 dealer card
    ...
    def dealer_choice():
        ...
        print("Dealer has total", sum(d_cards), "with the cards", d_cards)
    ...
    while sum(p_cards) < 21:
        k = input("Want to hit or stay?\nPress 1 for hit and 2 for stay: ")
        if k == "1":
            random.shuffle(deck)
            p_cards.append(deck.pop())
            print("You have a total of", sum(p_cards), "with the cards", p_cards)
